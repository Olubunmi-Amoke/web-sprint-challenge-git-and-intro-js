...
console.log('task 1 b:', artists[2].bio);
...
function listOfNames(artists) {
  let newArtistsArray = Array.from(artists);

  for (let i = 0; i < artists.length; i++ ) {
    newArtistsArray[i] = artists[i].name;
  }
...
function removeArtist(artists, number) {
  let newArtistsArray = Array.from(artists);
  let filteredArtists = newArtistsArray.filter((artist, index) => index != number )
...
console.log('task 6:', addArtist(artists, "Kuli", "1993 - 2100", "Javascript", "Nigeria", "music is life. love is the rhythm of love"));
...
function lotsOfArt(artists) {
  return artists
         ...
         .map(artistsWithLotsOfArt => artistsWithLotsOfArt.name);
...
function artistInfo(artists, name) {
  const artistByName = artists.filter(artist => artist.name === name)
  if (artistByName.length > 0) {
    return artistByName[0].bio;
  } else {
    return null;
  }
}

console.log('task 8:', artistInfo(artists, artists.name));
...
function artistByCountry(artists, nationality){
 const artistByNationality = artists.filter(artist => artist.nationality === nationality)
 const artistNamesWithSpecificNationality =  artistByNationality.map(artist => artist.name)
 return artistNamesWithSpecificNationality;

}

console.log('task 9:', artistByCountry(artists, 'German'));
